...
void parallel_for_flat(const double* a, const double* b, double* out,
    ...
    for (int t = 0; t < threads; ++t) {
        std::int64_t start = static_cast<std::int64_t>(t) * chunk;
...
FM_EXPORT int fm_add(const double* a, const double* b, double* out,
...
FM_EXPORT int fm_sub(const double* a, const double* b, double* out,
           std::int64_t rows, std::int64_t cols, int threads) {
    if (!a || !b || !out) {
...
FM_EXPORT int fm_transpose(const double* src, double* dst,
                 std::int64_t rows, std::int64_t cols, int threads) {
    if (!src || !dst) {
...
FM_EXPORT int fm_matmul(const double* a, const double* b, double* c,
    ...
    for (std::int64_t i = 0; i < n; ++i) {
        ...
        for (std::int64_t j = 0; j < k; ++j) bt[static_cast<std::size_t>(j * n + i)] = b_row[j];
    }
...
FM_EXPORT void* PyInit__fastmatrix_native(void) {
    return nullptr;
}
